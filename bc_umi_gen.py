...
def sim_seq(r, length, bc1, bc2, bc3, umi, tn, sp1, sp2):
    if r == 1:
        seq = random.sample(bc1, 1)[0] + sp1 + random.sample(bc2, 1)[0] + sp2 + random.sample(bc3, 1)[0] + random.sample(umi, 1)[0] + "T"*tn
        if length > len(seq):
...
def sim_fastq(n, r, length, bc1, bc2, bc3, umi ,tn, sp1, sp2):
    return("\n".join([sim_name(n, r), sim_seq(r, length, bc1, bc2, bc3, umi, tn, sp1, sp2), line3_template, sim_score(length)]))
# writing two files for r1 and 2
def sim_fastqs(file_name, n, length1, length2, bc1, bc2, bc3, umi, tn, sp1, sp2):
    with gzip.open(file_name + "_R1.fastq.gz", 'wt') as f1, gzip.open(file_name + "_R2.fastq.gz", 'wt') as f2:
        if n > 1000:
            ...
            ticks = []
        for i in range(n):
            seq2 = sim_fastq(i + 1, 2, length2, bc1, bc2, bc3, umi, tn, sp1, sp2)
            f2.write(seq2)
            f2.write("\n")
            seq1 = sim_fastq(i + 1, 1, length1, bc1, bc2, bc3, umi, tn, sp1, sp2)
            f1.write(seq1)
            f1.write("\n")
            if i in ticks:
                print(str(k) + "%...")
...
def main():
    ...
    sp1 = read_bc(sp1)[0]
    sp2 = read_bc(sp2)[0]
    umi = generate_umi(umi)
    
    print("generating " + str(n) + " fastq.gz read pairs...")
    sim_fastqs(output, n, l1, l2, bc1, bc2, bc3, umi, tn, sp1, sp2)
    print("done")
